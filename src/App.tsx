...
export default function App() {
  ...
  const [copyState, setCopyState] = useState<"none" | "text">("none");
...
<body>
  ...
  // Copy raw ceremony text to clipboard
  ...
  // Trigger default browser print dialog
  const handlePrint = () => {
    window.print();
  };
  ...
  return (
    <div className="bg-[#fcfaf7] text-[#1a1a1a] min-h-screen font-sans flex flex-col">
      ...
      {/* Visual Footer */}
      <footer className="border-t border-[#1a1a1a]/10 bg-white/40 flex flex-col sm:flex-row items-center justify-center gap-2 px-8 py-6 text-xs font-sans mt-auto no-print">
        <span className="text-[#1a1a1a]/60">Retrouvez nos chants sur YouTube :</span>
        <a 
          href="https://www.youtube.com/@latramprim" 
          target="_blank" 
          rel="noopener noreferrer"
          className="text-red-800 hover:text-red-950 font-bold transition-colors flex items-center gap-1.5 hover:underline"
        >
          <Youtube className="w-4 h-4 text-red-600" />
          <span>"Les chants des célébrations"</span>
        </a>
      </footer>
    </div>
  );
